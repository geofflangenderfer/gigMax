...
function combineExtractedAndCSVData() {
    ...
    for (let statementFilePath of statementFilePaths) {
        let tripIDs = getStatementTripIDs(statementFilePath);
        let statementJSON;
...
function saveCsvToJson(csvFilePath) {
    ...
    papaParse.parse(csvFileNames, {
        ...
        //dynamicTyping: true,
        skipEmptyLines: true,
        complete: (results) => (fs.writeFileSync(jsonFilePath, JSON.stringify(results.data, null, 4)))
    });
}
...
function getStatementJSONs() {
    let statementJSONs = getFilePathsArray(JSON_STATEMENT_DIR).map(filePath => (getJSON(filePath)));
